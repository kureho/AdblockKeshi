...
test('enabled=false なら何も登録しない（既定 OFF・プライバシー最小）', () => {
  const plan = popupShieldPlan({ enabled: false, pausedHosts: [] }, MULTI);
  assert.strictEqual(plan.register, false);
  assert.deepStrictEqual(plan.matches, []);
});

test('enabled=true なら全対象を登録', () => {
  const plan = popupShieldPlan({ enabled: true, pausedHosts: [] }, MULTI);
...
test('pausedHosts のサイトは matches から除外（サイト単位の一時停止）', () => {
  const plan = popupShieldPlan({ enabled: true, pausedHosts: ['streamtape.com'] }, MULTI);
  assert.strictEqual(plan.register, true);
...
test('全サイト pause なら register=false', () => {
  const plan = popupShieldPlan({ enabled: true, pausedHosts: ['streamtape.com', 'tokyomotion.net'] }, MULTI);
  assert.strictEqual(plan.register, false);
  assert.deepStrictEqual(plan.matches, []);
});
...
test('state 不正でも落ちない（堅牢）', () => {
  ...
  assert.strictEqual(popupShieldPlan({ enabled: true, pausedHosts: null }, MULTI).register, true);
});
