...
- ブロック統計・履歴表示
- 通知・更新案内
- ~~設定画面なし~~ → **v2.0 でメイン画面に 2 トグル追加**（広告 / 詐欺サイト）。
  **2026-09-18 に「設定」タブを新設**（kureho「設定のページ追加しよう。他と同じにしたほうが体験もいい」）＝
  他 21 本と同じ形（一番上に課金カード・下にサポート/バージョン）。
  ★**広告 / セキュリティの 2 トグルはメイン画面に置いたまま**（このアプリの主役なので設定へ移さない）。
  設定タブが持つのは 課金カード / ブロック停止中のサイト / お問い合わせ / このアプリについて / バージョン。
