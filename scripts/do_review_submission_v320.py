# [paid-approved-by-kureho] kureho承認済み・v3.2.0 reviewSubmission(最終提出)・ASC API・課金なし(DevProgram内)
"""広告消し v3.2.0 の reviewSubmission を作成 → item 追加 → submit する（最終提出）。

precheck ログ（2026-06-16-adblockkeshi-build20.json）書き出し済み・kureho 承認済み。
実行: python3 scripts/do_review_submission_v320.py
"""
import sys
...
APP_ID = "6774906945"
VERSION_ID = "55af8b82-ac2c-44d8-9894-57bafc4464a1"  # v3.2.0
...
def main() -> None:
    ...
    # 2) v3.2.0 を item として追加（既に入っていなければ）
    ...
    if has_version:
        print("[=] v3.2.0 は既に submission item に含まれている")
    else:
        ...
        print(f"[+] v3.2.0 を submission item に追加")
