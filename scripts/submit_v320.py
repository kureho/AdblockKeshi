...
WHATS_NEW = (
    ...
    "さらに「ポップアップ広告対策」を追加：タップ時の不要な誘導やポップアップ広告のスクリプトをブロックします（設定で ON）。\n"
...
REVIEW_NOTES = (
    ...
    "②「ポップアップ広告対策」拡張＝既知のポップアップ広告ネットワークのスクリプトをブロックし、"
    "タップ時の不要な遷移を抑制します。報告タブを使わない限り外部通信はありません。アプリ内課金（IAP）はありません。"
)
