...
/// 統合 ContentBlocker の combined を再生成して標準を reload する。
/// 基本保護の更新と同じタイミング（前面復帰時・BGTask）で呼ぶ。best-effort（失敗は無視）。
/// 4→3 統合後、報告由来ルールは標準 ContentBlocker に統合された（旧 reportedblocker は廃止）。
/// D-lite: 供給源はサーバ検証を通った global のみ（自己報告ファストレーンは廃止）。
