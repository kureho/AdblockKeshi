...
★**手元 `fastlane/metadata/ja/description.txt` は 2026-08-18 に live 値へ同期済み**。同期前は 2026-07-15 の版で、
`upload_to_app_store`（`skip_metadata: false`）を回すと live の 4.1.0 文面を巻き戻す状態だった。
**是正文面を手元に先入れしていない**のは、下の案が kureho 未承認だから。提出時に承認を取ってから入れる。
